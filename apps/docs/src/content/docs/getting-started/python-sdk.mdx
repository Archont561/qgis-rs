...
- Rust crate: `crates/qgis-sdk/Cargo.toml` — lib `_core` + bins `qgis-plugin`, `qgis-sdk`,
  reached from `py-packages/qgis-sdk/pyproject.toml` via `[tool.maturin].manifest-path`
