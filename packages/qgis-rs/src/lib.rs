...
impl PyExtent {
    ...
    #[getter]
    fn min_x(&self) -> f64 { self.inner.min_x }
    #[getter]
    fn min_y(&self) -> f64 { self.inner.min_y }
    #[getter]
    fn max_x(&self) -> f64 { self.inner.max_x }
    #[getter]
    fn max_y(&self) -> f64 { self.inner.max_y }
    ...
    fn contains(&self, x: f64, y: f64) -> bool { self.inner.contains(x, y) }

    fn intersects(&self, other: &PyExtent) -> bool { self.inner.intersects(&other.inner) }

    fn __str__(&self) -> String { self.inner.to_string() }
    fn __repr__(&self) -> String { format!("Extent({},{},{},{})", self.inner.min_x, self.inner.min_y, self.inner.max_x, self.inner.max_y) }

    fn __eq__(&self, other: &PyExtent) -> bool { self.inner == other.inner }
    ...
    fn to_list(&self) -> Vec<f64> {
        vec![self.inner.min_x, self.inner.min_y, self.inner.max_x, self.inner.max_y]
    }
}
...
impl PyCrs {
    ...
    #[staticmethod]
    fn wgs84() -> Self {
        Self { inner: Crs::wgs84() }
    }

    #[staticmethod]
    fn web_mercator() -> Self {
        Self { inner: Crs::web_mercator() }
    }
    ...
    fn __str__(&self) -> String { self.inner.auth_id().to_string() }
    fn __repr__(&self) -> String { format!("Crs({})", self.inner.auth_id()) }
    fn __eq__(&self, other: &PyCrs) -> bool { self.inner == other.inner }
}
...
impl PyTile {
    ...
    fn new(z: u32, x: u32, y: u32) -> Self {
        Self { inner: Tile::new(z, x, y) }
    }

    #[staticmethod]
    fn from_lon_lat(z: u32, lon: f64, lat: f64) -> Self {
        Self { inner: Tile::from_lon_lat(z, lon, lat) }
    }

    #[getter]
    fn z(&self) -> u32 { self.inner.z }
    #[getter]
    fn x(&self) -> u32 { self.inner.x }
    #[getter]
    fn y(&self) -> u32 { self.inner.y }

    fn bounds(&self) -> PyExtent {
        PyExtent { inner: self.inner.bounds() }
    }

    fn __str__(&self) -> String { format!("{}/{}/{}", self.inner.z, self.inner.x, self.inner.y) }
    fn __repr__(&self) -> String { format!("Tile(z={}, x={}, y={})", self.inner.z, self.inner.x, self.inner.y) }
    fn __eq__(&self, other: &PyTile) -> bool { self.inner == other.inner }
...
impl PyZoomRange {
    ...
    #[getter]
    fn min(&self) -> u32 { self.inner.min }
    #[getter]
    fn max(&self) -> u32 { self.inner.max }

    fn count(&self) -> u32 { self.inner.count() }
    ...
    fn __repr__(&self) -> String { format!("ZoomRange({}-{})", self.inner.min, self.inner.max) }
    fn __eq__(&self, other: &PyZoomRange) -> bool { self.inner == other.inner }
}
...
impl PyZoomLevelPlan {
    #[getter]
    fn zoom(&self) -> u32 { self.inner.zoom }
    #[getter]
    fn x_min(&self) -> u32 { self.inner.x_min }
    #[getter]
    fn x_max(&self) -> u32 { self.inner.x_max }
    #[getter]
    fn y_min(&self) -> u32 { self.inner.y_min }
    #[getter]
    fn y_max(&self) -> u32 { self.inner.y_max }

    fn tile_count(&self) -> u64 { self.inner.tile_count() }

    fn __repr__(&self) -> String {
        format!("ZoomLevelPlan(zoom={}, x={}..{}, y={}..{}, count={})",
            self.inner.zoom, self.inner.x_min, self.inner.x_max,
            self.inner.y_min, self.inner.y_max, self.inner.tile_count())
    }
}
...
impl PyTilePlan {
    ...
    fn zooms(&self) -> PyZoomRange {
        PyZoomRange { inner: self.inner.zooms }
    }

    fn level(&self, zoom: u32) -> PyZoomLevelPlan {
        PyZoomLevelPlan { inner: self.inner.level(zoom) }
    }

    fn levels(&self) -> Vec<PyZoomLevelPlan> {
        self.inner.levels().into_iter().map(|inner| PyZoomLevelPlan { inner }).collect()
    }
    ...
    fn __repr__(&self) -> String {
        format!("TilePlan(bounds={}, zooms={}-{}, tiles={})",
            self.inner.bounds, self.inner.zooms.min, self.inner.zooms.max, self.inner.tile_count())
    }
}
...
impl PyProject {
    ...
    fn layers(&self) -> PyResult<Vec<PyLayerSummary>> {
        self.inner.layers()
            .map(|layers| layers.into_iter().map(|l| PyLayerSummary { inner: l }).collect())
            .map_err(|e| PyValueError::new_err(e.to_string()))
    }

    fn render(&self, output: &str, width: Option<u32>, height: Option<u32>, extent: Option<&PyExtent>, crs: Option<&PyCrs>, dpi: Option<f64>) -> PyResult<PyRenderedMap> {
        use qgis_render::RenderSettings;
        let mut settings = RenderSettings::new(output)
            .map_err(|e| PyValueError::new_err(e.to_string()))?;
        if let Some(w) = width {
...
impl PyProjectInfo {
    #[getter]
    fn path(&self) -> String { self.inner.path.display().to_string() }
    #[getter]
    fn format(&self) -> String { self.inner.format.extension().to_string() }
    #[getter]
    fn size_bytes(&self) -> u64 { self.inner.size_bytes }
    #[getter]
    fn crs(&self) -> Option<PyCrs> {
        self.inner.crs.clone().map(|inner| PyCrs { inner })
    }
    #[getter]
    fn layer_count(&self) -> Option<usize> { self.inner.layer_count }
    #[getter]
    fn extent(&self) -> Option<PyExtent> {
        self.inner.extent.map(|inner| PyExtent { inner })
    }
    #[getter]
    fn note(&self) -> Option<String> { self.inner.note.clone() }

    fn __repr__(&self) -> String {
        format!("ProjectInfo(path={}, format={}, size={})", self.inner.path.display(), self.inner.format.extension(), self.inner.size_bytes)
    }
...
impl PyLayerSummary {
    ...
    #[getter]
    fn provider(&self) -> String { self.inner.provider.clone() }
    #[getter]
    fn crs(&self) -> Option<PyCrs> {
        self.inner.crs.clone().map(|inner| PyCrs { inner })
    }
    #[getter]
    fn feature_count(&self) -> Option<i64> { self.inner.feature_count }
    #[getter]
    fn geometry_type(&self) -> Option<String> { self.inner.geometry_type.clone() }

    fn __repr__(&self) -> String {
        format!("LayerSummary(name={}, provider={})", self.inner.name, self.inner.provider)
    }
}
...
impl PyRenderedMap {
    ...
    fn path(&self) -> String { self.inner.path.display().to_string() }
    #[getter]
    fn bytes(&self) -> u64 { self.inner.bytes }

    fn __repr__(&self) -> String {
        format!("RenderedMap(path={}, bytes={})", self.inner.path.display(), self.inner.bytes)
    }
}
...
impl PyRenderSettings {
    ...
    #[getter]
    fn dpi(&self) -> f64 { self.inner.dpi }

    fn with_extent(&self, extent: &PyExtent) -> Self {
        Self { inner: self.inner.clone().with_extent(extent.inner) }
    }

    fn with_crs(&self, crs: &PyCrs) -> Self {
        Self { inner: self.inner.clone().with_crs(crs.inner.clone()) }
    }

    fn with_dpi(&self, dpi: f64) -> Self {
        Self { inner: self.inner.clone().with_dpi(dpi) }
    }

    fn with_layers(&self, layers: Vec<String>) -> Self {
        Self { inner: self.inner.clone().with_layers(layers) }
    }

    fn __repr__(&self) -> String {
        format!("RenderSettings(width={}, height={}, dpi={})", self.inner.width, self.inner.height, self.inner.dpi)
    }
}
...
fn plan_tiles(bounds: &str, zoom: &str) -> PyResult<(u64, Vec<(u32, u32, u32, u32, u32, u64)>)> {
    ...
    let levels = plan.levels().into_iter().map(|l| (l.zoom, l.x_min, l.x_max, l.y_min, l.y_max, l.tile_count())).collect();
    Ok((total, levels))
}
