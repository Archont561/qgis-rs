...
impl ExtentWrapper {
    ...
    #[napi(getter)]
    pub fn min_x(&self) -> f64 { self.inner.min_x }
    #[napi(getter)]
    pub fn min_y(&self) -> f64 { self.inner.min_y }
    #[napi(getter)]
    pub fn max_x(&self) -> f64 { self.inner.max_x }
    #[napi(getter)]
    pub fn max_y(&self) -> f64 { self.inner.max_y }

    #[napi]
    pub fn width(&self) -> f64 { self.inner.width() }
    #[napi]
    pub fn height(&self) -> f64 { self.inner.height() }
    #[napi]
    pub fn is_valid(&self) -> bool { self.inner.is_valid() }
    #[napi]
    pub fn contains(&self, x: f64, y: f64) -> bool { self.inner.contains(x, y) }
    #[napi]
    pub fn intersects(&self, other: &ExtentWrapper) -> bool { self.inner.intersects(&other.inner) }
    #[napi]
    ...
    #[napi]
    pub fn equals(&self, other: &ExtentWrapper) -> bool { self.inner == other.inner }
}
...
impl CrsWrapper {
    ...
    #[napi(getter)]
    pub fn auth_id(&self) -> String { self.inner.auth_id().to_string() }
    ...
    #[napi]
    pub fn to_string(&self) -> String { self.inner.auth_id().to_string() }

    #[napi]
    pub fn equals(&self, other: &CrsWrapper) -> bool { self.inner == other.inner }
}
...
impl TileWrapper {
    ...
    #[napi(getter)]
    pub fn z(&self) -> u32 { self.inner.z }
    #[napi(getter)]
    ...
    pub fn bounds(&self) -> ExtentWrapper {
        ExtentWrapper { inner: self.inner.bounds() }
    }

    #[napi]
    pub fn to_string(&self) -> String { format!("{}/{}/{}", self.inner.z, self.inner.x, self.inner.y) }

    #[napi]
    pub fn equals(&self, other: &TileWrapper) -> bool { self.inner == other.inner }
}
...
impl ZoomRangeWrapper {
    ...
    #[napi(getter)]
    pub fn min(&self) -> u32 { self.inner.min }
    #[napi(getter)]
    pub fn max(&self) -> u32 { self.inner.max }
    ...
    #[napi]
    pub fn equals(&self, other: &ZoomRangeWrapper) -> bool { self.inner == other.inner }
}
...
impl ZoomLevelPlanWrapper {
    #[napi(getter)]
    pub fn zoom(&self) -> u32 { self.inner.zoom }
    #[napi(getter)]
    pub fn x_min(&self) -> u32 { self.inner.x_min }
    #[napi(getter)]
    pub fn x_max(&self) -> u32 { self.inner.x_max }
    #[napi(getter)]
    pub fn y_min(&self) -> u32 { self.inner.y_min }
    #[napi(getter)]
    pub fn y_max(&self) -> u32 { self.inner.y_max }

    #[napi]
    pub fn tile_count(&self) -> u64 { self.inner.tile_count() }
}
...
impl TilePlanWrapper {
    ...
    pub fn zooms(&self) -> ZoomRangeWrapper {
        ZoomRangeWrapper { inner: self.inner.zooms }
    }

    #[napi]
    pub fn level(&self, zoom: u32) -> ZoomLevelPlanWrapper {
        ZoomLevelPlanWrapper { inner: self.inner.level(zoom) }
    }

    #[napi]
    pub fn levels(&self) -> Vec<ZoomLevelPlanWrapper> {
        self.inner.levels().into_iter().map(|inner| ZoomLevelPlanWrapper { inner }).collect()
    }
    ...
    pub fn iter_tiles(&self) -> Vec<TileWrapper> {
        self.inner.iter().map(|inner| TileWrapper { inner }).collect()
    }
}
...
impl ProjectWrapper {
    ...
    #[napi(getter)]
    pub fn path(&self) -> String { self.inner.path().display().to_string() }
    ...
    #[napi]
    pub fn render(&self, output: String, width: Option<u32>, height: Option<u32>, dpi: Option<f64>) -> napi::Result<RenderedMapWrapper> {
        use qgis_render::RenderSettings;
        let mut settings = RenderSettings::new(&output).map_err(|e| napi::Error::from_reason(e.to_string()))?;
        if let Some(w) = width {
...
impl ProjectInfoWrapper {
    ...
    pub fn path(&self) -> String { self.inner.path.display().to_string() }
    #[napi(getter)]
    pub fn format(&self) -> String { self.inner.format.extension().to_string() }
    #[napi(getter)]
    pub fn size_bytes(&self) -> u64 { self.inner.size_bytes }
    #[napi(getter)]
    pub fn crs(&self) -> Option<CrsWrapper> {
        self.inner.crs.clone().map(|inner| CrsWrapper { inner })
    }
    #[napi(getter)]
    pub fn layer_count(&self) -> Option<u32> { self.inner.layer_count.map(|c| c as u32) }
    #[napi(getter)]
    pub fn note(&self) -> Option<String> { self.inner.note.clone() }
...
impl RenderedMapWrapper {
    ...
    pub fn path(&self) -> String { self.inner.path.display().to_string() }
    #[napi(getter)]
    pub fn bytes(&self) -> u64 { self.inner.bytes }
}
...
pub fn plan_tiles(bounds: String, zoom: String) -> napi::Result<TilePlanResult> {
    ...
    let levels = plan.levels().into_iter().map(|l| ZoomLevelInfo {
        zoom: l.zoom,
        x_min: l.x_min,
        x_max: l.x_max,
        y_min: l.y_min,
