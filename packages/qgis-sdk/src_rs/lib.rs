        ...
        if let Some(value) = py_dict.get_item(attr)? {
            ...
            let normalized = if let Ok(b) = value.extract::<bool>() {
                ...
                if let Ok(list) = value.extract::<Vec<String>>() {
                    let joined = list.into_iter().filter(|s| !s.trim().is_empty()).collect::<Vec<_>>().join(", ");
...
fn validate_plugin_structure(path: &str) -> PyResult<Vec<String>> {
    ...
    if !base.exists() {
        return Err(PyValueError::new_err(format!("path does not exist: {path}")));
    }
    ...
    if base.join("ui").exists() || base.join(base.file_name().unwrap_or_default()).join("ui").exists() {
        ...
        for ui_dir in ui_dirs {
            if ui_dir.exists() {
                let has_ui = std::fs::read_dir(&ui_dir)
                    .map(|mut d| d.any(|e| e.map(|e| e.path().extension().map(|ext| ext == "ui").unwrap_or(false)).unwrap_or(false)))
                    .unwrap_or(false);
                if !has_ui {
...
fn scaffold_plugin(
    ...
    if base.exists() {
        return Err(PyValueError::new_err(format!("directory already exists: {}", base.display())));
    }
...
"#,
        if plugin_type == "processing" { "True" } else { "False" },
        name = name,
    );
...
"#,
        ...
        std::fs::create_dir_all(base.join("src")).map_err(|e| PyValueError::new_err(e.to_string()))?;
        std::fs::write(
