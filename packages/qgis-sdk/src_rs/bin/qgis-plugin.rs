...
#[derive(Debug, Parser)]
#[command(name = "qgis-plugin", version, about = "QGIS plugin SDK — scaffold, build, test, and publish plugins at native speed (with UI + WebEngine support)")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
fn run(cmd: Command) -> Result<()> {
    match cmd {
        ...
        Command::Version => {
            println!("qgis-plugin {} (Rust-native, from qgis-sdk)", env!("CARGO_PKG_VERSION"));
...
"#,
    ...
    if with_ui {
        ...
        std::fs::write(dialogs_dir.join("__init__.py"), "from .main_dialog import MainDialog, make_declarative_dialog\n")?;
...
"#,
        name = name,
        has_provider = if args.r#type == "processing" { "True" } else { "False" },
    );
...
fn cmd_test(args: TestArgs) -> Result<()> {
    println!("Running tests (python={}, rust={})...", !args.rust, !args.python);
    println!("  pytest -q");
    if args.rust {
        println!("  cargo test");
    }
...
fn cmd_validate(args: ValidateArgs) -> Result<()> {
    ...
    for web_dir in [path.join("web"), path.join(path.file_name().unwrap_or_default()).join("web")] {
        if web_dir.exists() {
                    ...
                    if p.extension().map(|e| e == "html").unwrap_or(false) {
                        if let Ok(content) = std::fs::read_to_string(&p) {
                            if !content.contains("qwebchannel") && !content.contains("QWebChannel") {
                                errors.push(format!("{} missing qwebchannel.js reference", p.display()));
                            }
                        }
                    }
                }
    ...
    if errors.is_empty() {
        ...
        if path.join("ui").exists() || path.join(path.file_name().unwrap_or_default()).join("ui").exists() {
            println!("  UI: .ui files found");
        }
        if path.join("web").exists() || path.join(path.file_name().unwrap_or_default()).join("web").exists() {
...
fn cmd_rust_init(args: RustInitArgs) -> Result<()> {
    let path = args.path.unwrap_or_else(|| PathBuf::from("."));
    println!("Adding Rust acceleration to plugin in {}...", path.display());
...
fn cmd_ui_add_dialog(args: UiAddDialogArgs) -> Result<()> {
    let path = args.path.unwrap_or_else(|| PathBuf::from("."));
    println!("Adding UI dialog '{}' to plugin in {}...", args.name, path.display());
    ...
    let dialog_py = DIALOGS_MAIN_DIALOG_PY.replace("{name}", &pkg_dir.file_name().unwrap_or_default().to_string_lossy());
...
fn cmd_ui_add_web(args: UiAddWebArgs) -> Result<()> {
    let path = args.path.unwrap_or_else(|| PathBuf::from("."));
    println!("Adding WebEngine scaffolding to plugin in {}...", path.display());
...
mod chrono {
    ...
    impl Utc {
        ...
        pub fn format(&self, _fmt: &str) -> String { "20260918".to_string() }
    }
}
